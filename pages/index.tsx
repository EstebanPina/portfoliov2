...
export default function Home() {
  return (
    ...
    <HeaderMe></HeaderMe>
    <div className='flex flex-col gap-6 p-16 justify-start w-full min-h-screen bg-slate-950'>
      <h2 className='text-7xl font-bebas text-white'>Work Experience</h2>
      {work_experience.map((experience,index)=>{
        return(
          <div key={index} className='font-roboto bg-white/10 shadow-xl shadow-indigo-200/20 p-4 rounded-md text-white'>
            <h3 className='text-5xl font-bebas'>{experience.company}</h3>
            <p className='text-3xl font-bebas'>{experience.position}</p>
            <p className='text-xl font-bebas'>{experience.start} - {experience.end}</p>
            <div className='flex gap-2'>
            {experience.tech_stack.map((tech, index)=>{
              return(
                <span key={index} className='text-xl font-bebas'>{tech_dict[tech]}</span>
              )
            })}
            </div>
          </div>
